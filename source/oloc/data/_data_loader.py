...
test_cases = [
    ...
    "加 减 乘 除",
    "123+456-789*10/5",
    "123 plus 456 minus 789 multiply 10 divide 5",
    "pi+e-123",  # 测试符号 π 和 𝑒 的替换

    # 函数名保护测试
    "pow(2)+sqrt(4)-log(10)*exp(1)",
    "exp(pi)+pow(2)",
    "sqrt(e)+pow(pi)",
    ...
    "< >",
    "<pow>",
    "<123>123",  # 错误格式（不完全匹配自定义长无理数规则）
    "<A B>-?",
    "<number>3?",  # 错误格式（不符合规则）

    # 混合功能测试
    "pow(2)+<123>+pi-<自定义>+sqrt(4)+自然",
    "e<123>e<numb𝑒r>+pi",
    "exp(1)+<pow>-sqrt(<123>)",
    "<number>-?+sqrt(pi)+e",
    "<123>+?-e",
    ...
    "<><>",
    "<123><456>",
    "exp(<number>)-pow(<自定义>)+sqrt(<e>)",
    "pow(<123>)+sqrt(<456>natural)+e/pi",
    "<pow><pow><pow>",

    # 符号边界测试
    "123+456-789*10/5!",
    "(123+456)-sqrt(789)",
    "123*pow(456)+sqrt(789)",
    "e*pi-sqrt(123)+pow(456)+pi/e",
    "()[]{}<>",  # 测试括号和尖括号
    "自然=自然底数",  # 等号测试
